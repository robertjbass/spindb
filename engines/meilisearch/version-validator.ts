/**
 * Meilisearch version validation utilities
 * Handles version parsing, comparison, and compatibility checking
 */

/**
 * Parse a Meilisearch version string into components
 * Handles formats like "1.33.1", "1.33", "v1.33.1"
 */
...
/**
 * Check if a Meilisearch version is supported by SpinDB
...
/**
 * Compare two Meilisearch versions
 ...
 * Meilisearch snapshots are generally forward-compatible within major versions
 */
export function isVersionCompatible(
  ...
  if (backup.major > restore.major) {
    return {
      compatible: false,
      warning: `Cannot restore Meilisearch ${backupVersion} snapshot to ${restoreVersion} server. The backup is from a newer major version.`,
    }
  }
  ...
  return {
    compatible: true,
    warning: `Restoring Meilisearch ${backupVersion} snapshot to ${restoreVersion} server. Meilisearch will upgrade the snapshot format on next save.`,
  }
}
