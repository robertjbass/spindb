...
async function restoreTextBackup(
  ...
  return new Promise<RestoreResult>((resolve, reject) => {
    ...
    // Stream backup commands to valkey-cli stdin, SKIPPING comments and blank
    // lines. A spindb text backup opens with a `# Valkey backup generated by
    // SpinDB` header, and valkey-cli answers every such line with
    // `ERR unknown command '#'`. That was harmless while the exit code was the
    // verdict, but the reply text is the verdict now, so piping the file
    // verbatim fails every text restore on its own header. The Redis path has
    // filtered the same way since it started reading replies.
    ...
    proc.stdin.on('error', (error) => {
      ...
      if ((error as NodeJS.ErrnoException).code !== 'EPIPE') {
        streamError = new Error(
          `Failed to write to valkey-cli stdin: ${error.message}`,
        )
      }
      fileStream.destroy()
      lineReader.close()
