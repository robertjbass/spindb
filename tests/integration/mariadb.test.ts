/**
 * MariaDB System Integration Tests
 *
 * Tests the full container lifecycle with real MariaDB processes.
 * Uses pre-built binaries from hostdb (similar to PostgreSQL pattern).
 */
...
describe('MariaDB Integration Tests', () => {
  ...
  // leak assertion. mariadb-memory-budget_* does not match the
  ...
  before(async () => {
    ...
    console.log(`\n🦭 Using MariaDB version: ${DEFAULT_VERSION}`)

    // Download MariaDB binaries if not already installed
    console.log('\n📥 Ensuring MariaDB binaries are available...')
    ...
    containerName = leakGuard.track(generateTestName('mariadb-test'))
    clonedContainerName = leakGuard.track(
      generateTestName('mariadb-test-clone'),
    )
    renamedContainerName = leakGuard.track(
      generateTestName('mariadb-test-renamed'),
    )
    portConflictContainerName = leakGuard.track(
      generateTestName('mariadb-test-conflict'),
    )
  })
  ...
  it('should start the container', async () => {
    ...
    assert(ready, 'MariaDB should be ready to accept connections')
  ...
  it('should apply a memory budget (boots clean, caches trimmed)', async () => {
    ...
    const budgetName = leakGuard.track(
      generateTestName('mariadb-memory-budget'),
    )
  ...
  it('should query seeded data using executeQuery', async () => {
    ...
    // Verify first row data (MariaDB seed uses just first names)
  ...
  it('should create a new container from connection string (dump/restore)', async () => {
    ...
    assert(ready, 'Cloned MariaDB should be ready')
  ...
  it('should verify data persists after rename', async () => {
    ...
    assert(ready, 'Renamed MariaDB should be ready')
  ...
  it('should handle port conflict gracefully', async () => {
    ...
    try {
      // Try to create container on a port that's already in use
  ...
  it('should backup and restore with password-authenticated local root credentials', async () => {
    console.log(
      `\n🔐 Testing auth-aware MariaDB backup/restore on local containers...`,
    )
    ...
    const enableRootPasswordAuth = async (
      ...
      assert(
        ready,
        `MariaDB should be ready with password auth${lastError ? ` (${lastError})` : ''}`,
      )
    }
    ...
    try {
      ...
      assert(
        ...
        'Source MariaDB should be ready',
      )
      ...
      assert(
        ...
        'Target MariaDB should be ready',
      )
      ...
      assertEqual(
        ...
        'Restored MariaDB row count should match source under password auth',
      )
    } finally {
