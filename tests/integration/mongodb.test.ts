...
describe('MongoDB Integration Tests', () => {
  ...
  it('should create container without starting (--no-start)', async () => {
    ...
    await engine.initDataDir(containerName, '8.0', { superuser: '' })
  ...
  it('should start the container', async () => {
    ...
    // Create the user database (MongoDB creates it lazily)
  ...
  it('should seed the database with test data using runScript', async () => {
    ...
    const docCount = await getRowCount(
      ENGINE,
      testPorts[0],
      DATABASE,
      'test_user',
    ...
    console.log(`   ✓ Seeded ${docCount} documents using engine.runScript`)
  })
  ...
  it('should create a new container from connection string (dump/restore)', async () => {
    ...
    const engine = getEngine(ENGINE)
    await engine.initDataDir(clonedContainerName, '8.0', { superuser: '' })
    ...
    const dumpPath = join(tmpdir(), `mongodb-test-dump-${Date.now()}.archive`)
  ...
  it('should verify restored data matches source', async () => {
    ...
    console.log(`   ✓ Verified ${docCount} documents in restored container`)
  })
  ...
  it('should modify data using runScript inline JavaScript', async () => {
    console.log(`\n✏️  Deleting one document using engine.runScript with inline JavaScript...`)

    // Use runScriptSQL which internally calls engine.runScript with --sql option
    // For MongoDB, this is actually JavaScript, not SQL
    await runScriptSQL(
      containerName,
    ...
    assertEqual(docCount, EXPECTED_DOC_COUNT - 1, 'Should have one less document')

    console.log(`   ✓ Document deleted using engine.runScript, now have ${docCount} documents`)
  })
  ...
  it('should verify data persists after rename', async () => {
    ...
    console.log(`   ✓ Data persisted: ${docCount} documents`)
  })
