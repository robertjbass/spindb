...
- **The presence probe is time-bounded.** The listing behind `databaseExists()` (and so behind every `spindb start` and every backup on those engines) is raced against `DATABASE_PRESENCE_TIMEOUT_MS` (10 seconds). A listing that has not answered by then yields `'unknown'`, never `false`, so start and backup behave exactly as before instead of stalling a start or wake on a hung client. The timer is always cleared once the race settles (it is deliberately not unref'd, since that lets the event loop drain while a never-settling listing is awaited), and a listing that settles after the deadline (resolve or reject) is ignored.
- **A timed-out probe kills its client process.** A listing client left running with piped stdio keeps Node alive, and the CLI exits naturally on success, so a hung `psql` / `mariadb` / `mysql` / `cockroach sql` / `clickhouse client` would have kept `spindb start` from ever returning even after the probe gave up. `listDatabases` takes an optional `{ signal }`, honored only by those five engines (passed to `spawn`, which kills the child, and the listing rejects); the probe aborts it on timeout and still reports `'unknown'`. Every other engine and every existing caller is unchanged.
...
- **A CouchDB install carrying more than one bundled QuickJS build could pick the older one.** The `lib/couch_quickjs-*` directories were ordered as strings, and a string sort puts `couch_quickjs-3.10.0` BELOW `couch_quickjs-3.9.0`, so the first double-digit minor would have quietly downgraded the query server. The version components are now compared as numbers, highest wins, and a component that is not a plain integer (a prerelease suffix, anything unparsable) sorts last so it never outranks a real release.
- **A `restore --from-url` Redis copy that a MISCONF stopped now points at persistence instead of memory.** `MISCONF` and `OOM` both refuse the write and leave the same half-copied keyspace, so they were restated with the same sentence - which told the operator to raise `maxmemory` when the actual cause is that the target's RDB/AOF background save is failing. A MISCONF now says so and names what to check on the target server: its disk space and its `stop-writes-on-bgsave-error` / `save` settings. The OOM wording is unchanged, and both keep the server's own message in parentheses.
- **Every Valkey text restore failed on its own backup header.** 0.69.3 made the Valkey restore judge the server's reply text rather than the exit code, which is what it should do - but the Valkey path still piped the backup file to `valkey-cli` verbatim, header comments included, and the server answers a `# Valkey backup generated by SpinDB` line with `ERR unknown command '#'`. Harmless while nobody read the replies, fatal once they were read: a restore that wrote every key still reported the header's error and failed. Comment and blank lines are now skipped before the file reaches `valkey-cli`, exactly as the Redis path has always done, so the reply text again describes only real commands. RDB restores were never affected.
- **A copy that hit a write refusal mid-batch reported whichever error arrived first.** One pipeline can come back with several failures, and an earlier per-key complaint (a payload-format refusal, say) was reported ahead of the `OOM`/`MISCONF` that is the reason the copy stopped and the thing the operator has to act on. The target-side refusal is now preferred when one is present; with no refusal in the batch, the first failure is reported exactly as before.
...
- **A Redis or Valkey migration between two different engine families always failed, and could not be made to work by choosing versions.** `restore --from-url` moved a keyspace with `SCAN` + `DUMP` -> `RESTORE`, which carries the source server's own serialization. `RESTORE` verifies the RDB version stamped in that payload and refuses anything outside the format it knows, and Redis and Valkey no longer stamp from the same number space. Measured against live servers on 2026-09-09: Redis 7.2 writes RDB 11, Redis 8.x writes 12, Upstash (which reports `redis_version:8.4.0`) writes 14, Valkey 8.0 writes 11, and Valkey 9.0 writes 80. So a Valkey 9 source could not be copied into ANY Redis, a Redis 8 or Upstash source could not be copied into Redis 7.2 or into Valkey, and every one of those failed on the first batch with `ERR DUMP payload version or checksum are wrong` and zero keys moved. The advice spindb printed - provision the target at the same major version or newer - was unactionable, because no version of the other engine accepts the payload. The copy now falls back automatically: the moment the target refuses the payload format (or the source does not implement `DUMP` at all, as a serverless provider may not), it switches for the rest of the run to a type-aware walk that READS each key with `TYPE`/`GET`/`HSCAN`/`SSCAN`/`ZSCAN`/`LRANGE`/`XRANGE`/`PTTL` and REWRITES it with `SET`/`HSET`/`SADD`/`ZADD`/`RPUSH`/`XADD`/`PEXPIRE`, in cursor-sized chunks so one enormous key cannot blow up a single request. Strings, hashes, lists, sets, sorted sets and streams cross over with their values, their order, their stream ids and their TTLs intact; binary keys and binary values stay binary. `DUMP`/`RESTORE` remains the fast path whenever the target accepts it, and nothing about a same-family migration changes. `--json` reports which one ran as `strategy: "dump-restore" | "logical"`. Module types (`ReJSON-RL`, `TSDB-TYPE`, `MBbloom--`) have no portable read/write pair, so they are counted and named in `skipped`/`skippedTypes` rather than silently dropped.
- **A source that accepted the TCP connection and then never spoke RESP hung the migration forever.** The reply parser waits for a CRLF before it decides anything, and the bytes a TLS-only endpoint sends back over a plain `redis://` connection (a TLS alert record) contain none - so the copy sat with commands in flight until something outside spindb killed it, with no error, no output and no exit. Two guards: a reply whose first byte is not one RESP2 can produce now fails immediately, naming the byte and pointing at `rediss://`, and the socket carries an idle deadline that fails the connection when it goes silent with commands outstanding.
- **A malformed reply took the whole process down instead of failing the command.** The parser threw from inside the socket's `data` handler, where nothing catches it - an uncaught exception, so `--json` printed nothing at all and the exit code was all a caller had. It is now surfaced as a normal connection failure.
- **A `-` reply with no text produced an `Error` whose message was the empty string**, which serializes to nothing and reads downstream as no error at all. It now carries text.
- **The common handshake refusals say what to change.** `WRONGPASS`, `NOAUTH`, `AUTH` against a server with no password, a two-argument `AUTH` sent to a pre-ACL server, and a `/N` database suffix the source does not have each keep the server's own message and gain one sentence naming the fix. A rejected handshake also no longer leaks the socket.
...
- **A restore that could not create every object now says so, in `--json` too.** `pg_restore` keeps going past an object it cannot create, so a dump that references an extension the target does not have failed every `CREATE TABLE` whose column default called it, skipped every `COPY` into those tables, and still ended with spindb printing `success: true` and exiting 0. That is how a Supabase migration lost 19 tables without a single visible error: `CREATE EXTENSION "uuid-ossp"` was unavailable, every default calling `extensions.uuid_generate_v4()` failed, 218 grants named a `role "authenticated"` that did not exist, and `pg_restore` reported `errors ignored on restore: 819` on its way to a zero exit code. `engines/postgresql/restore-diagnostics.ts` reads what the tool actually said (its own `pg_restore: error:` lines, the `psql:<file>:<line>: ERROR:` lines of the plain-SQL branch, whose path is read as psql echoed it and so may carry a Windows drive letter, its warnings, and the ignored-errors summary), and `core/restore-outcome.ts` turns that into one verdict every restore path shares. The `--json` object gains `status` (`completed` or `completed_with_errors`, always present) plus, ONLY when objects failed, `restoreErrorCount`, `restoreErrors` (deduplicated, capped at 200), `restoreWarningCount`, `restoreIgnoredErrors` and `restoreErrorsTruncated`. `success` stays `true` and the exit code stays 0 for a partial restore, because the database exists and holds what could be restored and a rollback would throw that away: check `status`, not the exit code, to know whether everything arrived. Additive on purpose - no existing field changed shape, so layerbase-desktop and layerbase-cloud keep working unchanged.
- **`spindb restore --pre-sql <file>`** runs a SQL file against the target database after it is created and BEFORE the restore, which is the fix for the failure above: create the extension, the roles, or the function a column default calls, then restore. It runs inside the same rollback scope as the restore, so a shim that fails takes down the database THIS restore created and never one you already had; with `--into-existing` it runs before that restore too, where there is nothing to roll back. PostgreSQL, MySQL and MariaDB (the engines whose client can be told to keep its chatter off stdout, which `--json` requires). A missing file fails before anything else runs, and `preSqlApplied: true` is reported in `--json`.

### Fixed

- **`code` was undefined on EVERY successful PostgreSQL restore.** `engines/postgresql/restore.ts` spread `promisify(exec)`'s `{stdout, stderr}` into its result and never set `code`, so every caller's `result.code === 0` check was false and a perfectly clean restore printed "Restore completed with warnings" 100% of the time. Because that warning was normal, the one that meant something was invisible. Both success paths now set `code: 0` explicitly, and all three paths attach the parsed diagnostics.
- **A failed restore is now distinguished from a partial one.** The old rule failed only on a literal `FATAL` in stderr, which is why a restore that lost 19 tables passed for a success. A `FATAL`, or a non-zero exit with no object errors to explain it, is now a failure that throws and rolls back; object errors are a partial restore that reports itself. The same classifier is used by `spindb restore`, `spindb create --from-url`, the interactive restore handlers, and `core/backup-restore.ts`, so the four paths cannot drift apart again - and each of them reports a failed restore as a failure: an engine's `restore()` RESOLVES on one (it returns the non-zero exit rather than throwing), so a path that only asks whether objects failed hands its caller `success: true` on a `FATAL` and nothing downstream catches it.
- **`--json` output could not be parsed exactly when something went wrong.** The "Warnings:" block after a restore wrote to STDOUT unconditionally, so a partial restore emitted human text ahead of the JSON object. Every such block is now gated on `!options.json`, including the remote-dump warnings `create --from-url` printed before its own JSON object (they are reported as `remoteDumpWarnings` in the payload instead, so nothing is lost).
- **A failed `restore --from-url` printed nothing at all in `--json` mode** and, without `--json`, split the label onto stderr and the detail onto stdout so no pipe kept them together. It now prints `{ error, errorName?, errorCode?, phase: "remote-dump", tool }` in JSON mode and writes both halves to stderr otherwise. The connection-string validation, missing-backup-file, RDB-state and dump-retry exits nearby gained the JSON branch they were missing.
- **A failed Redis keyspace migration printed `{}`.** The catch used `(error as Error).message` on a rejection that is not always an `Error`, and `JSON.stringify` drops `undefined`, so a `--json` caller got an empty object. `describeThrown` (`core/error-handler.ts`) describes anything a catch can receive - an `Error`, a thrown string, a rejected `{ code }` object, null - and always yields a non-empty message. The Redis failure now reports `{ error, errorName?, errorCode?, phase: "redis-keyspace-copy", keysCopied }`, with the progress counter hoisted so it can say how far the copy got.
- **An InfluxDB restore where some tables wrote and others did not is now reported as partial** rather than being read as an unexplained failure under the stricter rule above (which would have rolled back a database holding most of the data). Its per-table errors moved from `stdout` to `stderr`, where every caller looks for them.

### Documentation

- CHEATSHEET.md documents `--pre-sql`, the new `--json` fields, and the "a restore that lost objects says so" contract, with the Supabase shim recipe. README.md lists `--pre-sql` alongside `--into-existing`.
...
- **`spindb restore <mysql-container> --from-url mysql://...` now works when the server on the other end is MariaDB.** It could not work before, and no flag could make it: the dump ran `mysqldump`, and `mysqldump` 9 cannot even authenticate against a MariaDB server, because MySQL 9 removed the `mysql_native_password` client plugin that MariaDB's root user still uses. The restore died on `Got error: 2059: Authentication plugin 'mysql_native_password' cannot be loaded` before reading a single table, on a `mysql://` URL that gave no hint the source was not MySQL. The dump tool now follows the SOURCE server instead of the container being restored into, which is what PostgreSQL has always done (`validateDumpCompatibility` swaps in a `pg_dump` that can read the remote major version). The MySQL family extends that rule with flavor, because MySQL and MariaDB share both a wire protocol and the `mysql://` scheme, so nothing but the server itself can say which one is answering.
- **The source is identified from its own greeting, before any credentials are sent.** `core/server-handshake.ts` opens a TCP connection, reads the initial handshake packet a MySQL-protocol server sends unprompted, and closes. It never writes to the socket, so it works against a server that requires TLS (TLS is negotiated in the client's reply) and it cannot fail authentication. `parseMysqlWireHandshake` is pure and unit tested against fixture packets, including a greeting split across two reads, an `ERR` packet sent instead of a greeting, and a non-MySQL protocol. `classifyMysqlFamilyServer` sees through the `5.5.5-` replication prefix MariaDB 10 announces itself with, which would otherwise read as MySQL 5.5. A source that cannot be probed is treated as unknown and takes the unchanged `mysqldump` path, so a probe failure is never the reason a dump does not happen. This lives in `core/`, not in the MySQL engine: it is the shape any engine family with non-interchangeable client tools needs.
- **A MariaDB dump is converted for the MySQL target instead of failing statement by statement.** Even with the right tool, a MariaDB dump is not MySQL SQL. MariaDB 11.4 made `utf8mb4_uca1400_ai_ci` the default database collation, so nearly every `CREATE TABLE` names a collation MySQL has never had, and each one fails; MariaDB still emits `NO_AUTO_CREATE_USER` in the `SET sql_mode` around triggers and routines, which MySQL 8+ answers with ERROR 1231. `engines/mysql/dump-normalize.ts` maps each `uca1400` collation to its MySQL UCA 9.0.0 counterpart (`ai_ci`, `as_cs` and `as_ci` exactly; `ai_cs` to `as_cs`, the closest MySQL has; `nopad_` variants to their padded equivalent; anything else to the default), maps `utf8mb3` uca1400 collations to `utf8mb3_unicode_ci`, or to `utf8mb3_bin` when the suffix asked for case sensitivity (`as_cs`, `ai_cs`, and their `nopad_` forms), since MySQL's UCA 9.0.0 collations are utf8mb4 only and `utf8mb3_bin` is the one utf8mb3 collation that keeps case sensitive comparison, strips `NO_AUTO_CREATE_USER` with the comma bookkeeping that leaves a valid mode list, and drops the MariaDB sandbox directive. It runs line by line over a stream, so a multi-gigabyte dump is never held in memory, and it reports per-rule counts that are printed after the dump and carried into `--json` as `remoteSource`, because a silent rewrite of someone's schema is not acceptable. What is deliberately NOT rewritten: sequences and MariaDB-only column types (`UUID`, `INET4`, `INET6`, `VECTOR`), which have no MySQL equivalent and are left to fail with the server's own error naming the object; `json_valid()` CHECK constraints need no help, since MySQL has `json_valid()`.
- **The remote dump no longer runs an arbitrary installed version of its client tool.** A MySQL 9.7.2 container took its remote dump through `mysql-9.6.0/bin/mysqldump`, with both installed. Nothing was wrong with the 9.6 binary: `configManager.getBinaryPath('mysqldump')` keeps ONE path per tool name with no version dimension, so it returns whichever was registered first and still exists on disk, and the remote dump path asked it directly while the local backup path had its own version-aware lookup. `core/mysql-family-binary-resolver.ts` is now that lookup for both, the MySQL-family twin of `core/pg-binary-resolver.ts`: the requested version exactly, then the newest install on that version line (matched on version segments, so `11.8` never matches `11.80`), then the newest installed, with the globally registered path as the last resort. `spindb restore`, `pull`, `create` and the interactive menu all pass the target container's version, and a MariaDB source picks a `mariadb-dump` on the source's own version line.
- **A machine with no MariaDB installed downloads the client tools rather than failing.** Restoring a MariaDB source into MySQL needs `mariadb-dump` even though MariaDB is not the engine being used, so spindb fetches MariaDB's default line through the same binary download path `spindb engines download` uses. If that download is unavailable the error names the source version and points at `spindb engines download mariadb`, instead of surfacing as a raw spawn failure.
- **A server whose greeting declares an empty packet is reported as an unprobeable source instead of taking the process down.** `parseMysqlWireHandshake` read the protocol byte out of the packet payload without checking that the header had declared one, so a packet header carrying a payload length of zero left it reading `undefined` and throwing a `TypeError` while formatting the "unexpected protocol version" message. That throw happened inside the socket's `data` listener, which makes it an uncaught exception rather than a rejected promise, so the best-effort `probeMysqlFamilyServer` wrapper could not catch it and a probe of a misbehaving front end crashed the command instead of falling back. An empty payload is now classified before any byte of it is read, and the data listener wraps the parser so no future parse failure can escape as an uncaught exception either: both end as a failed read, which the probe already reports as an unknown flavor and the unchanged `mysqldump` path. Covered by a fixture packet and by a socket test serving that packet to the real probe.
- The `selectInstalledVersion` segment-matching test now proves what it claims. Its fixture installed only `11.80.1` and `10.11.15` and asked for `11.8`, so `11.80.1` was both the substring match and the newest install: a resolver that matched on substrings would have passed it. It now asks for `9.6` with `9.6.0` and `9.60.1` installed, where the two rules disagree, and asserts the older `9.6.0` wins.
- Proven end to end against real binaries by a new `mysql-mariadb-interop` integration test (`pnpm test:engine mysql-mariadb-interop`, and in the MySQL CI job, which now downloads both engines): a real MariaDB 11.8 source with its real default `uca1400` collation, a `json_valid()` CHECK constraint and a trigger, dumped through the MySQL engine and restored into a real MySQL 9 container, asserting the rows arrive (non-ASCII included), the collation is `utf8mb4_0900_ai_ci` on the target, the trigger fires, and the CHECK constraint survives unconverted.
...
- **A failed remote dump during `spindb create` now names the tool that actually ran instead of always blaming `pg_dump`.** The retry loop around `dumpFromConnectionString` labelled every engine's failure as `pg_dump error:`, so importing into a MariaDB container from a MySQL 8.4 source reported a `pg_dump` error for a failure that happened inside `mariadb-dump`. That sent you looking at the wrong tool, and at PostgreSQL, on a create that never involved either. The label is now taken from the engine being created, so you see `mariadb-dump error:`, `mysqldump error:`, or `pg_dump error:` as appropriate, and an engine with no dump utility of its own reads as a neutral `dump error:`. The matching "not installed" prompt names the same tool and points at the right `spindb engines download <engine>`, and MariaDB's client tool names are now recognized by the missing-tool detection that previously only knew the PostgreSQL and MySQL ones.
...
- Formatting-only release: applied `pnpm format` (prettier) across the repo to
  clear accumulated drift. No functional changes.
...
- **TypeDB `createDatabase` / `dropDatabase` authenticated with the hardcoded `admin`/`password` default instead of the database's saved credentials.** Every other TypeDB console operation (`backup`/`database export`, `restore`/`database import`) already loads the saved credentials, but these two console calls passed no auth and so always used the built-in default. When the admin password has been rotated out-of-band (Layerbase Cloud rotates it to a per-database password during setup), the default no longer authenticates and both ops fail with a generic "Failed to drop database" / "Failed to create database" - which broke restore's drop+recreate step on a credential-rotated TypeDB. Both now load the saved credentials via the same `loadCredentials(...)` path as backup/restore. Found by the Layerbase Cloud restore-drill.
