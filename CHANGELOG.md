...
- **A failed restore is now distinguished from a partial one.** The old rule failed only on a literal `FATAL` in stderr, which is why a restore that lost 19 tables passed for a success. A `FATAL`, or a non-zero exit with no object errors to explain it, is now a failure that throws and rolls back; object errors are a partial restore that reports itself. The same classifier is used by `spindb restore`, `spindb create --from-url`, the interactive restore handlers, and `core/backup-restore.ts`, so the four paths cannot drift apart again.
- **`--json` output could not be parsed exactly when something went wrong.** The "Warnings:" block after a restore wrote to STDOUT unconditionally, so a partial restore emitted human text ahead of the JSON object. Every such block is now gated on `!options.json`.
