#!/usr/bin/env tsx
/**
 * Generate a Valkey database with sample data.
 *
 * Usage:
 *   pnpm generate:db valkey [container-name] [--port <port>]
 */
...
async function main(): Promise<void> {
  ...
  console.log('Valkey Database Generator')
  console.log('=========================\n')
  ...
  if (!isReady) {
    console.error('Error: Valkey did not become ready in time')
    process.exit(1)
  }
  ...
  // Execute each Valkey command individually (supports # comments)
