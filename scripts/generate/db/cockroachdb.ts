#!/usr/bin/env tsx
/**
 * Generate a CockroachDB database with sample data.
 *
 * Usage:
 *   pnpm generate:db cockroachdb [container-name] [--port <port>]
 */
...
async function main(): Promise<void> {
  ...
  console.log('CockroachDB Database Generator')
  console.log('==============================\n')
  ...
  console.log('Waiting for CockroachDB to be ready...')
  const isReady = await waitForReady(containerName, ['--', '-e', 'SELECT 1'])

  if (!isReady) {
    console.error('Error: CockroachDB did not become ready in time')
    process.exit(1)
  }
  ...
  const seedResult = runContainerCommand(containerName, ['-e', seedContent])
