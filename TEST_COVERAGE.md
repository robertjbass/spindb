...
|----------|--------|------|-------------------------|
| Engine lifecycle (create/start/stop/delete) | All 21 engines | None | High |
...
| User management (users create) | 14 of 21 engines (excludes SQLite, DuckDB, QuestDB, TypeDB, InfluxDB, Weaviate, TigerBeetle) | N/A (excluded engines have no user management) | High |
...
- **Engine lifecycle** (create, start, stop, delete, list, info) -- integration tests for all 21 engines
...
Exceptions: ClickHouse skips Windows (no binaries available). LibSQL skips Windows (no binaries available). FerretDB skips Windows (postgresql-documentdb startup issues).
